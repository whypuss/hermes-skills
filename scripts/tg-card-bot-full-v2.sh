...
# 2. 安裝 Python 庫（用 --break-system-packages）
echo "[2/5] 安裝 Python 庫..."
/opt/venv/bin/pip install aiogram==3.13.1 python-dotenv==1.0.1 --break-system-packages 2>/dev/null || \
pip3 install aiogram==3.13.1 python-dotenv==1.0.1 --break-system-packages 2>/dev/null || \
/opt/venv/bin/pip install aiogram==3.13.1 python-dotenv==1.0.1 2>/dev/null
...
def init_db():
    with get_db() as db:
        db.execute("CREATE TABLE IF NOT EXISTS products (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT, price REAL, stock INTEGER, description TEXT, card_content TEXT, category TEXT DEFAULT '軟件')")
        db.execute("CREATE TABLE IF NOT EXISTS orders (id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER, product_id INTEGER, paid INTEGER DEFAULT 0, created_at TEXT)")
        db.commit()

def get_products(category=None):
    with get_db() as db:
        if category:
            return db.execute("SELECT id,name,price,stock,description FROM products WHERE category=? AND stock>0", (category,)).fetchall()
        return db.execute("SELECT id,name,price,stock,description FROM products WHERE stock>0").fetchall()

def get_product(pid):
    with get_db() as db:
        return db.execute("SELECT id,name,price,stock,description,card_content FROM products WHERE id=?", (pid,)).fetchone()

def deduct_stock(pid):
    with get_db() as db:
        db.execute("UPDATE products SET stock=stock-1 WHERE id=? AND stock>0", (pid,))
        db.commit()

def add_order(uid, pid):
    from datetime import datetime
    with get_db() as db:
        db.execute("INSERT INTO orders (user_id,product_id,paid,created_at) VALUES (?,?,1,?)",(uid,pid,datetime.now().isoformat()))
        db.commit()
...
async def s(m): init_db(); await m.answer("👋 歡迎！選分類：", reply_markup=menu_kb())
...
async def o(m):
    if m.from_user.id != ADMIN_ID: return
    with get_db() as db:
        rs = db.execute("SELECT id,user_id,product_id,paid,created_at FROM orders ORDER BY id DESC LIMIT 20").fetchall()
...
async def h(m):
    ...
    s = AW[uid]
    if s["s"] == "name":
        AW[uid] = {"s":"price", "n":m.text}
        await m.answer(f"📦 {m.text}\n價格：")
    elif s["s"] == "price":
        try:
            AW[uid] = {"s":"stock","n":s["n"],"p":float(m.text)}
            await m.answer("庫存：")
        except:
            await m.answer("❌ 數字")
    elif s["s"] == "stock":
        try:
            AW[uid] = {"s":"card","n":s["n"],"p":s["p"],"st":int(m.text)}
            await m.answer("卡密（每行一組）：")
        except:
            await m.answer("❌ 整數")
    elif s["s"] == "card":
        with get_db() as db:
            db.execute("INSERT INTO products (name,price,stock,card_content) VALUES (?,?,?,?)",(s["n"],s["p"],s["st"],m.text.strip()))
            db.commit()
...
async def cc(c):
    ps = get_products(c.data.replace("cat_",""))
...
async def b(c):
    p = get_product(int(c.data.replace("buy_","")))
